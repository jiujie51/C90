...
# allow CFLAGS and LDFLAGS to be modified from the make command line
CFLAGS := $(filter-out $(ECOS_CFLAGS_REMOVE),$(CFLAGS)) $(ECOS_CFLAGS_ADD)
LDFLAGS := $(filter-out $(ECOS_LDFLAGS_REMOVE),$(LDFLAGS)) $(ECOS_LDFLAGS_ADD)
...
# Patch for Win32 version of Code Sourcery tool chain install path which cause "multiple target patterns" error
ifeq ($(HOST),CYGWIN)
CODESOURCERY_PATH = $(dir $(shell which $(CC)))
ifneq "$(CODESOURCERY_PATH)" "/cygdrive/c/CodeSourcery/SourceryLite/bin/"
$(error "Please install CodeSourcery toolchain to path : c:\CodeSourcery\SourceryLite")
endif
else
CODESOURCERY_PATH = /opt/im/mipsel-sde-elf-4.4.3/bin
endif
...
%.o.d : %.c $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	...
	@sed -e '/^ *\\/d' -e "s#.*: #$@: #" $(@:.o.d=.tmp) > $@
	@sed -i 's/c\:\\codesourcery\\sourcerylite\\bin\\/\/cygdrive\/c\/codesourcery\/sourcerylite\/bin\//g' $@
	@rm $(@:.o.d=.tmp)

%.o.d : %.cxx $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
%.o.d : %.cpp $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
%.o.d : %.S $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
%.d : %.c $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
%.d : %.cxx $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
%.d : %.cpp $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
%.d : %.S $(REPOSITORY)/pkgconf/ecos_supp.mak
ifeq ($(HOST),CYGWIN)
	@mkdir -p `cygpath -w "$(dir $@)" | sed "s@\\\\\\\\@/@g"`
else
	@mkdir -p $(dir $@)
...
TESTS := $(TESTS:.c=)
TESTS := $(TESTS:.S=)
tests.stamp: $(foreach target,$(TESTS),$(target).d $(if $(suffix $(target)),,$(PREFIX)/tests/$(PACKAGE)/$(target)$(EXEEXT)))
ifneq ($(strip $(TESTS)),)
...
clean:
	...
	@find . -type f -name "*.o.d" -print | xargs rm -f
	@find . -type f -name "*.a.deps" -print | xargs rm -f
	@find . -type f -name "*.a.stamp" -print | xargs rm -f
