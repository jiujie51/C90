...
//#NT# Use GXIMG_SCALE_BILINEAR to due to IME: path1 does not support ISD. JiraID: NA51023-296.
    ...
    if (FlowPB_GetFullCropDisp()){
		...
		//hwmem_memset(pMultiDst->pImg[0]->PxlAddr[0], 0x00, pMultiDst->pImg[0]->Width*pMultiDst->pImg[0]->LineOffset[0]);
		//hwmem_memset(pMultiDst->pImg[0]->PxlAddr[1], 0x80, pMultiDst->pImg[0]->Width*pMultiDst->pImg[0]->LineOffset[1]/2);
		hwmem_close();	  
		#endif
    }
