	...
	if ((uiFlag & STAMP_OPERATION_MASK) == STAMP_AUTO) {
		...
		default:
			if ((uiFlag & STAMP_POS_END_MASK) == STAMP_POS_END) {
				g_MovieStampPos[uiVidEncId].uiX = uiImageWidth - uiStampWidth * strlen(pStampInfo->pi8Str) - STAMP_WIDTH_TOLERANCE ;
				g_MovieStampPos[uiVidEncId].uiY = uiImageHeight - pStampInfo->ui32DstHeight;
				//debug_msg("---------cj g_MovieStampPos[uiVidEncId].uiX :%d \r\n ",g_MovieStampPos[uiVidEncId].uiX );
				...
				g_MovieStampPos[uiVidEncId].uiX = uiImageWidth - uiStampWidth * (strlen(pStampInfo->pi8Str) + 2); // 1 font width gap
				g_MovieStampPos[uiVidEncId].uiY = uiImageHeight - (pStampInfo->ui32DstHeight * 3) / 2; // 1/2 font height gap
				//debug_msg("---------cj 22 g_MovieStampPos[uiVidEncId].uiX :%d \r\n ",g_MovieStampPos[uiVidEncId].uiX );
...
UINT32 MovieStamp_GetBufAddr(UINT32 uiVidEncId, UINT32 blk_size)
{
		...
		CHKPNT;
		debug_msg("zjf create private pool MovieStamp_%d successful \r\n",uiVidEncId);
...
void MovieStamp_DestroyBuff(void)
{
	...
	for (i=0;i<VIDEO_IN_MAX;i++) {
		if(g_MovieStampPool[i] != NVTMPP_VB_INVALID_POOL){
			...
			CHKPNT;
			debug_msg("zjf destroy private pool MovieStamp_%d successful \r\n",i);
...
void MovieStamp_UpdateData(void)
{
	...
	for (i = 0; i < VIDEO_IN_MAX; i++) {
		if ((g_uiMovieStampSetup[i] & STAMP_SWITCH_MASK) == STAMP_ON) {
			...
			//debug_msg("zjf ----- watermark is date time -----\r\n");
		...
		if(Ipl_id<IPL_ID_MAX_NUM){
			...
			//debug_msg("g_cMovieStampStr[pathid] is %s \r\n",g_cMovieStampStr[i]);
		}
		//NMR_VdoEnc_GetRCStr(i, &g_cMovieStampStr[i][0], MOVIE_STAMP_MAX_LEN);
			...
			//debug_msg("watermark is %s \r\n",g_cMovieStampStr[i][0]);
			//debug_msg("g_MovieStampInfo[pathid]->pi8Str is %s \r\n",g_MovieStampInfo[i].pi8Str);
			//debug_msg("\r\n");
			//DBGD(i);
			//DBGD(g_MovieStampInfo[i].ui32FontWidth);
			//DBGD(g_MovieStampInfo[i].ui32DstHeight);
			...
			DBG_DUMP("time2: %d us\r\n", Perf_GetCurrent() - uiTime);
				   ...
				   //DBGD(g_VsFontIn[i].pFont);
				   //DBGD(g_VsOsdWidth[i]);
				   //DBGD(g_VsOsdHight[i]);
				   //DBGD(g_MovieStampPos[i].uiX);
				   //DBGD(g_MovieStampPos[i].uiY);
