...
//#NT#Parameter number must be larger then 0. Fixed mantis bug id:0107729
	...
	if (par == WIFI_APP_MODE_PLAYBACK) {
		UxCtrl_SetShow(&UIMenuWndWiFiMobileLinkOK_PlaybackModePanelCtrl,TRUE);
		UIPlay_SetRootDirType(ROOTDIR_TYPE_MOVIE);//zjf add ---默认进入循环视频目录
...
INT32 WiFiCmd_OnExeEthcamChange(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
CHKPNT;
	UINT32 par = 0;
	if (paramNum > 0) {
		...
		WifiCmd_Done(WIFIFLAG_ETHCAM_CHG_DONE, WIFIAPP_RET_PAR_ERR);
		DBG_ERR("too few parameter!\r\n");
		return NVTEVT_CONSUME;
	}
	...
	switch (par) {
		case  WIFI_APP_SET_FRONT :
		{
				SysSetFlag(FL_DUAL_CAM,DUALCAM_FRONT);
				WifiCmd_Done(WIFIFLAG_ETHCAM_CHG_DONE, WIFIAPP_RET_OK);
				
		}
		break;
		
		case WIFI_APP_SET_BEHIND:
			if(EthCamNet_GetEthLinkStatus(0) == ETHCAM_LINK_UP) 
			{
				...
				WifiCmd_Done(WIFIFLAG_ETHCAM_CHG_DONE, WIFIAPP_RET_STATE_ERR);
			}
...
if(EthCamNet_GetEthLinkStatus(0) != ETHCAM_LINK_UP   ) 
{
	...
	debug_msg("WiFiCmd_OnExePipStyle:%d \r\n", Data);
...
INT32 WiFiCmd_OnAppSessionClose(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	Fun_CloseWifiTimer();
	Ux_PostEvent(NVTEVT_SYSTEM_MODE, 2, System_GetState(SYS_STATE_CURRMODE), SYS_SUBMODE_NORMAL );//SYS_SUBMODE_NORMAL
...
EVENT_ENTRY UIWifiCmdObjCmdMap[] = {
	...
	{NVTEVT_WIFI_EXE_ETHCAM_CHG, WiFiCmd_OnExeEthcamChange },
