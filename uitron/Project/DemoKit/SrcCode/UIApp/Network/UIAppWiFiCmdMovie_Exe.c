...
debug_msg("\r\n cj  curStatus:%d startRec:%d  uiNotifyAPPStatus:%d  %d %d %d\r\n",curStatus, startRec, uiNotifyAPPStatus,paramArray[0],paramArray[1], paramNum  );
	if (startRec) {
...
#if  1//(WIFI_FINALCAM_APP_STYLE == ENABLE)
		{
			BOOL bReOpen = FALSE;
...
INT32 WiFiCmd_OnExeCyclicRec(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	if (WiFiCmd_GetStatus() != WIFI_MOV_ST_RECORD) {
		CHKPNT;
		UI_SetData(FL_MOVIE_CYCLIC_REC_MENU, Data);
