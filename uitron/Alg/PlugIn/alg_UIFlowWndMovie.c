...
//kevin_lca
#if 0 // (AV_LCA_FUNC == ENABLE) // cj 20200615
{
	extern UINT32 UIFlowWndMovie_AV_LCA_Draw(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray);
	UIFlowWndMovie_AV_LCA_Draw(pCtrl, paramNum, paramArray);
}
#endif
	return NVTEVT_CONSUME;
}
...
UINT32 UIFlowWndMovie_AV_LCA_Draw(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	#if(defined(_NVT_ETHREARCAM_RX_))
	extern  URECT      gMovieOsdDispCord;
	UIScreen ScreenObj = *paramArray;
	URECT DispCord = {0,0,320,240};
	DispCord =gMovieOsdDispCord;


	extern UINT32 g_LCAWarnRslt[2];
	extern UINT32 g_VehicleNum;
	extern URECT g_VehicleRect[10];
	extern IPOINT g_LCAInstallCoord[5];
	

	UINT32 x1 = 0, y1 = 0, x2 = 0, y2 = 0, idx = 0;	
	
	//debug_msg("(%d,%d,%d,%d)\r\n",DispCord.x,DispCord.y,DispCord.w,DispCord.h);

	CHAR InfoStr[50] = {0};
	sprintf(InfoStr, "%d num",g_VehicleNum);
	GxGfx_SetTextStroke((const FONT *)&gDemoKit_Font, FONTEFFECT_HIGHLIGHT2, SCALE_1X);
	GxGfx_Text(((DC **)ScreenObj)[GxGfx_OSD], 180*DispCord.w/320+DispCord.x, 150*DispCord.h/240+DispCord.y, (CHAR *)InfoStr);

	#if 0
	INT32 i = 0;
	for(i=0;i<5;i++)
	{
		debug_msg("coord=(%d,%d)\r\n",g_LCAInstallCoord[i].x,g_LCAInstallCoord[i].y);
	}
	#endif


	GxGfx_SetShapeColor(CLRID_IDX_RED, CLRID_IDX_RED, 0);

	x1 = g_LCAInstallCoord[0].x * DispCord.w/1280+DispCord.x;
	y1 = g_LCAInstallCoord[0].y * DispCord.h/720+DispCord.y;
	
	x2 = g_LCAInstallCoord[1].x * DispCord.w/1280+DispCord.x;
	y2 = g_LCAInstallCoord[1].y * DispCord.h/720+DispCord.y;
	GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x2, y2);
	//debug_msg("(%d,%d,%d,%d)\r\n", x1, y1, x2, y2);


	x2 = g_LCAInstallCoord[2].x * DispCord.w/1280+DispCord.x;
	y2 = g_LCAInstallCoord[2].y * DispCord.h/720+DispCord.y;
	GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x2, y2);


	x2 = g_LCAInstallCoord[3].x * DispCord.w/1280+DispCord.x;
	y2 = g_LCAInstallCoord[3].y * DispCord.h/720+DispCord.y;
	GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x2, y2);


	x2 = g_LCAInstallCoord[4].x * DispCord.w/1280+DispCord.x;
	y2 = g_LCAInstallCoord[4].y * DispCord.h/720+DispCord.y;
	GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x2, y2);
	...
	GxGfx_SetShapeColor(CLRID_IDX_WHITE, CLRID_IDX_WHITE, NULL);
	for (idx = 0; idx < g_VehicleNum; idx++) 
	{
		x1 = g_VehicleRect[idx].x*DispCord.w/1280+DispCord.x;
		y1 = g_VehicleRect[idx].y*DispCord.h/720+DispCord.y;
		x2 = x1 + g_VehicleRect[idx].w*DispCord.w/1280;
		y2 = y1 + g_VehicleRect[idx].h*DispCord.h/720;

		//debug_msg("(%d,%d,%d,%d)(%d,%d)\r\n", x1, y1, x2, y2,DispCord.w,DispCord.h);
		
		
		//GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x2, y2);

		//GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], 0, DispCord.h/2, DispCord.w, DispCord.h/2);

		/**/GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x2, y1);
		GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x2, y1, x2, y2);
		GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y2, x2, y2);
		GxGfx_Line(((DC **)ScreenObj)[GxGfx_OSD], x1, y1, x1, y2);

	}


	if(g_LCAWarnRslt[0]==1)
	{
		GxGfx_SetShapeColor(CLRID_IDX_YELLOW, CLRID_IDX_YELLOW, NULL);
		GxGfx_FillRect(((DC**)ScreenObj)[GxGfx_OSD],0,0,20,DispCord.h+DispCord.y);

	}
	else if(g_LCAWarnRslt[0]==2)
	{
		GxGfx_SetShapeColor(CLRID_IDX_RED, CLRID_IDX_RED, NULL);
		GxGfx_FillRect(((DC**)ScreenObj)[GxGfx_OSD],0,0,20,DispCord.h+DispCord.y);
	...
	if(g_LCAWarnRslt[1]==1)
	{
		GxGfx_SetShapeColor(CLRID_IDX_YELLOW, CLRID_IDX_YELLOW, NULL);
		GxGfx_FillRect(((DC**)ScreenObj)[GxGfx_OSD],DispCord.w+DispCord.x-20,0,DispCord.w+DispCord.x,DispCord.h+DispCord.y);

	}
	else if(g_LCAWarnRslt[1]==2)
	{
		GxGfx_SetShapeColor(CLRID_IDX_RED, CLRID_IDX_RED, NULL);
		GxGfx_FillRect(((DC**)ScreenObj)[GxGfx_OSD],DispCord.w+DispCord.x-20,0,DispCord.w+DispCord.x,DispCord.h+DispCord.y);
